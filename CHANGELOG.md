...
- **1:1 verification with confidence bands.** `verify_faces(feat1, feat2,
  threshold=..., bands=...)` wraps `compute_similarity` with a decision
  threshold and labels the score with a confidence band ('high', 'medium',
  'low', 'no_match' by default), returning a `VerificationResult`. Bands are
  independent of the threshold so identity-verification flows get a natural
  manual-review zone.
- **Top-k search in the FAISS vector store.** `FAISS.search_topk(embedding,
  k, threshold)` returns up to *k* `(metadata, similarity)` pairs sorted by
  decreasing similarity, for 1:N identification and duplicate screening.
  `BaseStore` gains a non-abstract `search_topk` that raises
  `NotImplementedError`, so existing backends keep working.
- **Spoofing and quality adapters for `FaceAnalyzer`.** `SpoofingPredictor`
  (wraps any `BaseSpoofer`, defaults to `MiniFASNet`) and `QualityPredictor`
  (wraps any `BaseQualityEstimator`, defaults to `EDifFIQA`) implement the
  `BaseAttribute` interface, so one `analyze()` call can produce detection,
  embedding, liveness, and quality. `Face` gains `is_real` and
  `spoofing_confidence` fields.
- **Blink detection for active liveness.** `uniface.liveness` computes
  per-eye aspect ratios (EAR) from dense `FaceMesh` landmarks
  (`eye_aspect_ratio`, `compute_eye_aspect_ratios`) and counts blinks across
  frames with the stateful `BlinkDetector` — a challenge a printed photo or
  screen replay cannot answer.
